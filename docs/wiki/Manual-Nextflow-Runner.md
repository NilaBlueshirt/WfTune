# Manual Nextflow runner

Complete the [campaign preparation](https://github.com/NilaBlueshirt/WfTune/wiki/Campaign-Guide) first. This guide
covers the manual Nextflow path; run its commands from a frozen checkout with
the configured shared paths and root/benchmark-user sessions.
...
- **`start_monitor.sh` warns about another WfTune version.** Runs already
  recorded for this venue came from a different release, or from before
  version recording. The step still opens the trial, but analysis rejects a
  campaign that mixes versions unless explicitly overridden. If the mix is
  unintended, stop with `abort_run.sh` before launching the pipeline, then
  restore the campaign's release or start a new campaign; see
  [version recording](https://github.com/NilaBlueshirt/WfTune/wiki/Campaign-Guide#version-recording).
